...
console.log('params=' + params.args[0])
...
var listen = server.listen(port, function(request, response) {// 监听端口
	...
	console.log('args=' + args.reqMethod)
...
function echarts(args) {
    ...
    page.open("about:blank", function(status) {// 空白页
        ...
        var pageBody = page.evaluate(function(args) {
			...
			var itemStyle = '{"normal":{"color" : new echarts.graphic.LinearGradient(0, 0, 0, 1, [ '+ '{offset : 0,	color : "#3398DB"}, {offset : 0.5,color : "#3398DB"}, {offset : 1,color : "#3398DB"} ])}}';
